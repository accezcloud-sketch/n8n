...
Self-hosted n8n on a VPS via Docker. Runs four blog auto-deploy workflows that generate posts with Gemini, fetch Unsplash images (where the site uses a cover), push to GitHub, and announce on Facebook:
...
Visit `https://n8n.accez.cloud`. Sign in with **the same owner account credentials** that existed on the source instance (the zip preserves the user table). All four workflows should be visible in the workflow list, all `Inactive`, with their credentials already attached and unbroken (no yellow "credential not found" badges).
...
For each of the four workflows: open it in the editor, click the **Inactive** toggle in the top-right header to flip it to **Active**. n8n shows a confirmation toast and a "Next execution: <timestamp>" line appears.

The schedule rule (`daysInterval: 2`) is **calendar-anchored** — it fires on every odd-numbered day of the month at the configured hour, regardless of when activation happens. So all four workflows automatically align on the same calendar days. The only thing activation time controls is whether *today* is included as the first fire:

- Activated before 09:00 on an odd day → Accez, CloudElite, Veganster, and Wady all fire today
- Activated 09:00–12:00 on an odd day → CloudElite, Veganster, and Wady fire today; Accez fires next odd day
- Activated 12:00–15:00 on an odd day → Veganster and Wady fire today
- Activated 15:00–18:00 on an odd day → only Wady fires today
- Activated after 18:00 on an odd day, or any time on an even day → none fire today; first fires happen on the next odd day

After that, all four fire together on every odd-numbered day. Caveat: month boundaries (e.g. Jan 31 → Feb 1) cause occasional 1-day gaps instead of 2.
...
- `workflows/wady-blog-auto-deploy.json` — Wady Cloud bilingual (EN+AR) blog workflow (fallback import)
...
4. **Import each workflow JSON** via Workflows → Import from File:
   ...
   - `workflows/wady-blog-auto-deploy.json`
5. **Recreate all eight credentials** in Credentials → Create new. Names must match exactly:

| Name | Type | Used by | Where to get it |
|---|---|---|---|
| `Google Sheets account` | Google Sheets OAuth2 API | All 4 | Google Cloud Console → OAuth client |
| `Google Gemini(PaLM) Api account` | Google Gemini (PaLM) API | All 4 | https://aistudio.google.com/apikey |
| `GitHub account` | GitHub API | All 4 | GitHub → Settings → Developer settings → Personal access tokens (classic, `repo` scope) |
| `Header Auth account 2` | HTTP Header Auth | Accez, CloudElite, Veganster (Unsplash) | Unsplash app → access key. Header name: `Authorization`, value: `Client-ID YOUR_KEY` |
...
| `Facebook Wady Cloud Page Token` | Facebook Graph API | Wady Cloud Facebook | Page access token for the Wady Cloud page (id `1014705518402959`) |
